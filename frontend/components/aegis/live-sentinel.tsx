...
import { FIREHOSE_LOGS } from "@/lib/aegis-data"
...
const REASONING_TEMPLATES: ActionCard["reasoning"][] = [
  [
    ...
    "→ Audio fingerprint: commentator timbre matches NBC broadcast",
    ...
    const tick = () => {
      ...
      if (tpl.lvl === "MATCH" && Math.random() > 0.55) {
        ...
        const action: ActionCard = {
          id: counterRef.current,
          ts: nowTs(),
          matchId: ["LAKERS_WARRIORS_001", "CELTICS_HEAT_044", "NUGGETS_SUNS_018"][
            ...
            {actions.map((a) => (
              ...
              >
                <GlowCard className="p-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-md border border-foreground/15 bg-white/40 hover:bg-white/70 px-3 py-2 text-[11.5px] font-medium transition-colors">
